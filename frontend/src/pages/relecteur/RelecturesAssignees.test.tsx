...
function relecture(champs: Partial<RelectureAssignee>): RelectureAssignee {
  return { id: 7, exerciceId: 412, sessionId: 6, sessionTitre: 'Conception d’API REST', statut: 'EN_ATTENTE_RELECTURE', lien: null, note: null, commentaire: null, rendueAt: null, ...champs }
}
...
function afficher(liste: RelectureAssignee[], rendu: (appel: Appel) => { statut: number; corps: unknown }) {
  ...
  const appels = simulerApi({
    ...
    'POST /api/relectures/7/debut': () => {
      etat = [relecture({ statut: 'EN_COURS_DE_RELECTURE', lien: LIEN })]
      return { corps: etat[0] }
    },
    'POST /api/relectures/7': (appel) => {
      const reponse = rendu(appel)
      if (reponse.statut === 200) etat = [relecture({ statut: 'EN_COURS_DE_RELECTURE', lien: LIEN, note: 14, rendueAt: '2026-09-25T11:00:00Z' })]
      return reponse
    },
  })
...
describe('RelecturesAssignees (spécification 08, EF6)', () => {
  ...
  it('commence la relecture avec l’en-tête X-Etudiant-Id et révèle le lien (RG23)', async () => {
    ...
    expect(screen.getByText('Exercice n° 0412')).toBeInTheDocument()
    ...
    const debut = appels.find((a) => a.chemin === '/api/relectures/7/debut')
    expect(debut?.entetes['X-Etudiant-Id']).toBe('11')
    expect(await screen.findByRole('link', { name: LIEN })).toBeInTheDocument()
  })

  it('désigne la relecture par son propre id et affiche le numéro de l’exercice (étape 3)', async () => {
    afficher([relecture({ statut: 'EN_COURS_DE_RELECTURE' })], () => ({ statut: 200, corps: {} }))
    // L'autre relecteur a commencé : l'exercice est « en cours », mais cette relecture-ci est à commencer.
    expect(await screen.findByRole('button', { name: 'Commencer la relecture' })).toBeInTheDocument()
  })
  ...
  it('rend la note et le commentaire, puis la ligne passe « Rendue · 14 / 20 »', async () => {
    const { appels, utilisateur } = afficher([relecture({ statut: 'EN_COURS_DE_RELECTURE', lien: LIEN })], () => ({
      statut: 200,
      corps: { id: 7, exerciceId: 412, statut: 'RELU', note: 14, commentaire: 'Bien', rendueAt: '2026-09-25T11:00:00Z' },
    }))
    ...
    const rendu = appels.find((a) => a.chemin === '/api/relectures/7')
