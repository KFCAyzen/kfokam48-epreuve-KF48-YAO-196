# D2 — Modèle de données

Les tables, leurs colonnes et leurs cardinalités, **avec les noms exacts des migrations Flyway** (`backend/src/main/resources/db/migration/`). Chaque table correspond à une entité JPA du paquet `entity`, jamais exposée en JSON (B3).

```mermaid
erDiagram
    promotion ||--o{ etudiant : "regroupe"
    promotion ||--o{ session_cours : "organise"
    session_cours ||--o{ presence : "enregistre"
    etudiant ||--o{ presence : "est présent"
    session_cours ||--o{ exercice : "reçoit"
    etudiant ||--o{ exercice : "dépose (auteur)"
    exercice ||--o| relecture : "est relu par"
    etudiant ||--o{ relecture : "relit (relecteur)"
    etudiant ||--o| blocage_code : "peut être bloqué"

    promotion {
        bigint id PK
        varchar_100 nom UK "NOT NULL"
    }
    etudiant {
        bigint id PK
        varchar_100 nom "NOT NULL"
        bigint promotion_id FK "NOT NULL"
    }
    session_cours {
        bigint id PK
        varchar_200 titre "NOT NULL"
        bigint promotion_id FK "NOT NULL"
        varchar_6 code UK "NOT NULL, jamais réutilisé (RG5)"
        timestamptz ouverture_at "NOT NULL"
        timestamptz expiration_at "NOT NULL, ouverture + 15 min (RG1)"
        timestamptz cloturee_at "NULL tant que non clôturée (RG21)"
    }
    presence {
        bigint id PK
        bigint session_id FK "NOT NULL"
        bigint etudiant_id FK "NOT NULL"
        varchar_10 source "ETUDIANT ou FORMATEUR (RG8)"
        timestamptz marquee_at "NOT NULL"
    }
    exercice {
        bigint id PK
        bigint session_id FK "NOT NULL"
        bigint etudiant_id FK "NOT NULL, auteur"
        varchar_500 lien "NOT NULL, URL http(s) (RG11)"
        varchar_25 statut "DEPOSE, EN_ATTENTE_RELECTURE, EN_COURS_DE_RELECTURE, RELU"
        timestamptz depose_at "NOT NULL"
        timestamptz modifie_at "NULL, dernier remplacement du lien"
    }
    relecture {
        bigint exercice_id PK, FK "même id que l'exercice (1-1, Q6)"
        bigint relecteur_id FK "NOT NULL, jamais l'auteur (RG2)"
        integer note "NULL tant que non rendue, 0 à 20 (RG3)"
        varchar_2000 commentaire "NULL tant que non rendue (RG17)"
        timestamptz assignee_at "NOT NULL"
        timestamptz commencee_at "NULL, bloque le remplacement (RG23)"
        timestamptz rendue_at "NULL, définitive une fois posée (RG18)"
    }
    blocage_code {
        bigint etudiant_id PK, FK
        integer echecs_consecutifs "NOT NULL, défaut 0"
        timestamptz bloque_jusqua "NULL, fin du blocage de 2 min (RG7)"
    }
```

## Cardinalités

| Relation | Cardinalité | Lecture |
|---|---|---|
| promotion → etudiant | 1 — 0..* | un étudiant appartient à une seule promotion |
| promotion → session_cours | 1 — 0..* | une session est ouverte pour une seule promotion |
| session_cours → presence | 1 — 0..* | au plus une présence par étudiant et par session (RG4) |
| etudiant → presence | 1 — 0..* | un étudiant est présent à plusieurs sessions |
| session_cours → exercice | 1 — 0..* | au plus un exercice par étudiant et par session (RG10) |
| etudiant → exercice | 1 — 0..* | l'étudiant auteur |
| exercice → relecture | 1 — 0..1 | 0 tant que l'exercice est `DEPOSE` (RG15), exactement 1 ensuite (RG13) |
| etudiant → relecture | 1 — 0..* | l'étudiant relecteur ; il n'existe pas de table `relecteur` |
| etudiant → blocage_code | 1 — 0..1 | ligne créée au premier code faux (RG7) |

## Contraintes posées par la migration

| Table | Contrainte | Règle |
|---|---|---|
| `promotion` | `UNIQUE (nom)` | — |
| `session_cours` | `UNIQUE (code)` | RG5 |
| `presence` | `UNIQUE (session_id, etudiant_id)` | RG4 |
| `presence` | `CHECK (source IN ('ETUDIANT', 'FORMATEUR'))` | RG8 |
| `exercice` | `UNIQUE (session_id, etudiant_id)` | RG10 |
| `exercice` | `CHECK (statut IN ('DEPOSE', 'EN_ATTENTE_RELECTURE', 'EN_COURS_DE_RELECTURE', 'RELU'))` | D4 |
| `relecture` | `CHECK (note BETWEEN 0 AND 20)` | RG3 |
| `relecture` | clé primaire `exercice_id`, clé étrangère vers `exercice(id)` | RG13 |

**Types.** Mermaid n'accepte pas d'espace ni de parenthèse dans un type : `varchar_100` se lit `VARCHAR(100)`, `timestamptz` se lit `TIMESTAMP WITH TIME ZONE`. Les identifiants `bigint` sont générés par la base (`GENERATED BY DEFAULT AS IDENTITY`), sauf `relecture.exercice_id` et `blocage_code.etudiant_id`, qui reprennent la clé de leur parent.

**Le formateur n'a pas de table** : il n'a pas de compte (Q1), aucune donnée ne le désigne.
