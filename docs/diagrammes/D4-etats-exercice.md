...
La colonne `exercice.statut` (D2) ne prend que ces quatre valeurs. Le cycle demandé, « déposé → en attente de relecture → relu », est complété par l'état « en cours de relecture », qui rend observable le début de la relecture (Q13, RG23). **Depuis l'étape 3**, chaque exercice a deux relecteurs (RG13) : il n'est `RELU` que lorsque les deux relectures sont rendues ; avec une seule, sa note est provisoire (RG25).
...
stateDiagram-v2
    ...
    choix_relecteur --> EN_ATTENTE_RELECTURE : 1 ou 2 relecteurs tirés (RG14)
    ...
    EN_ATTENTE_RELECTURE --> EN_ATTENTE_RELECTURE : lien remplacé, second relecteur (RG15, RG23)
    EN_ATTENTE_RELECTURE --> EN_COURS_DE_RELECTURE : un relecteur commence ou rend (RG16)

    EN_COURS_DE_RELECTURE --> EN_COURS_DE_RELECTURE : une relecture rendue, note provisoire (RG25)
    EN_COURS_DE_RELECTURE --> RELU : les deux relectures rendues (RG25)
    ...
    note left of RELU
        État final : note retenue = moyenne des deux,
        chaque relecture est définitive (RG18, RG25)
    end note
```
...
| `EN_ATTENTE_RELECTURE` | Un relecteur commence ou rend | `EN_COURS_DE_RELECTURE` | l'appelant est l'auteur · n'est pas le relecteur assigné | `403 AUTO_RELECTURE` · `403 RELECTEUR_NON_ASSIGNE` |
| `EN_COURS_DE_RELECTURE` | Relecture rendue `POST /api/relectures/{id}` | `RELU` si c'est la seconde, sinon inchangé avec une note provisoire | note hors 0–20 ou non entière · commentaire vide · auteur · pas le relecteur assigné | `400 NOTE_INVALIDE` · `400 COMMENTAIRE_INVALIDE` · `403 AUTO_RELECTURE` · `403 RELECTEUR_NON_ASSIGNE` |
